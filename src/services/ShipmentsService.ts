...
export abstract class ShipmentsService {
    ...
    /**
     * Delete shipments
     * Deletes every current shipment matching the given filters.
     * Accepts the same filters as GET /shipments.
     *
     * At least one filter is required: a request with no filters set is
     * rejected to avoid deleting every shipment for the account by accident.
     *
     * In some cases the request will be rejected (400 validation error) if
     * there is too much data to delete. Narrow the provided filters and try again.
     *
     * @param data Request data
     * @param options Additional operation options
     * @returns void
     */
    public deleteShipments(
        data?: {
            /**
             * Free-text search term used to match shipment IDs in the returned results.
             *
